...
class EstimationResourceIT {
    ...
    void getAllEstimations() throws Exception {
        ...
        restEstimationMockMvc
            ...
            .andExpect(jsonPath("$.[*].id").value(hasItem(estimation.getId().toString())))
    ...
    void getEstimation() throws Exception {
        ...
        restEstimationMockMvc
            ...
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.id").value(estimation.getId().toString()))
    ...
    void getNonExistingEstimation() throws Exception {
        // Get the estimation
        restEstimationMockMvc.perform(get(ENTITY_API_URL_ID, UUID.randomUUID().toString())).andExpect(status().isNotFound());
    }
    ...
    void putNonExistingEstimation() throws Exception {
        int databaseSizeBeforeUpdate = estimationRepository.findAll().size();
        estimation.setId(UUID.randomUUID());
    ...
    void deleteEstimation() throws Exception {
        ...
        restEstimationMockMvc
            .perform(delete(ENTITY_API_URL_ID, estimation.getId().toString()).accept(MediaType.APPLICATION_JSON))
